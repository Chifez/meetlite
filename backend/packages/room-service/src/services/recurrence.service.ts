 ...
 */
export function generateRRULE(recurrence: any, startDate: Date): string {
  const options: any = {
    dtstart: startDate,
  ...
  switch (recurrence.pattern) {
    ...
    case 'weekly':
      ...
      if (recurrence.daysOfWeek && recurrence.daysOfWeek.length > 0) {
        ...
        options.byweekday = recurrence.daysOfWeek.map((day: number) => dayMap[day]);
      } else {
 ...
 */
export function isRecurrenceOccurrence(date: Date, recurrence: any, startDate: Date): boolean {
  try {
 ...
 */
export function parseRRULE(rruleString: string, dtstart: Date): any {
  try {
    const rule = rrulestr(rruleString) as any;
    ...
    if (options.freq === RRule.DAILY) {
      ...
      if (options.byweekday) {
        recurrence.daysOfWeek = options.byweekday.map((day: any) => {
 ...
 */
export function validateRecurrence(recurrence: any): any {
  ...
  if (recurrence.pattern === 'weekly' && recurrence.daysOfWeek) {
    ...
    const invalidDays = recurrence.daysOfWeek.some(
      (day: any) => !Number.isInteger(day) || day < 0 || day > 6
    );
    if (invalidDays) {
      return {
        valid: false,
  ...
  try {
    ...
    const existingDates = new Set(
      existingInstances.map(
        (inst: any) => new Date(inst.scheduledTime).toISOString().split('T')[0]
      )
    );
