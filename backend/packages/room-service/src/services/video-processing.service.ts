 ...
 */
export const getVideoMetadata = async (input: Buffer | string): Promise<any> => {
    ...
    const processInput = async () => {
      try {
        if (Buffer.isBuffer(input)) {
          ...
          tempFile = path.join(
            os.tmpdir(),
            `video_${Date.now()}_${Math.random().toString(36).substring(2, 11)}.mp4`
          );
 ...
 */
export const generateThumbnail = async (input: Buffer | string, options: any = {}): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    let inputPath: string;
    let tempFile: string | null = null;
    ...
    const processThumbnail = async () => {
      try {
        ...
        outputPath = path.join(
          os.tmpdir(),
          `thumb_${Date.now()}_${Math.random().toString(36).substring(2, 11)}.jpg`
        );
 ...
 */
export const extractAudioFromVideo = async (input: Buffer | string, options: any = {}): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    let inputPath: string;
    let tempFile: string | null = null;
    let outputPath: string | null = null;
    ...
    const processAudio = async () => {
      try {
        ...
        outputPath = path.join(
          os.tmpdir(),
          `audio_${Date.now()}_${Math.random().toString(36).substring(2, 11)}.mp3`
        );
 ...
 */
export const assessVideoQuality = (metadata: any): string => {
