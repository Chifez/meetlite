...
export class YjsController {
  private yjsSyncService: any;
  private io: any;

  constructor(yjsSyncService: any, io: any) {
   ...
   */
  async handleSyncStep1(socket: any, data: any) {
    try {
   ...
   */
  async handleUpdate(socket: any, data: any) {
    try {
   ...
   */
  async handleAwarenessUpdate(socket: any, data: any) {
    try {
   ...
   */
  async handleAwarenessQuery(socket: any, data: any) {
    try {
   ...
   */
  async handleDisconnect(socket: any, roomId: string) {
    try {
      const userId = socket.user?.userId;
