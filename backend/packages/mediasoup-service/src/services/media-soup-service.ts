...
export class MediaSoupService {
   ...
   */
  async validateRoomAccess(roomId: string, userId: string, token: string) {
    try {
   ...
   */
  async addParticipant(roomId: string, userId: string, userInfo: any) {
    try {
   ...
   */
  async createWebRtcTransport(roomId: string, direction: string, userId: string) {
    try {
      const roomData = this.rooms.get(roomId);
      if (!roomData) {
        throw new Error(`Room not found: ${roomId}`);
   ...
   * Connect transport
   */
  async connectTransport(roomId: string, transportId: string, dtlsParameters: any, userId: string) {
    try {
   ...
   * Pause producer
   */
  async pauseProducer(roomId: string, producerId: string, userId: string) {
    try {
      const roomData = this.rooms.get(roomId);
      if (!roomData || !roomData.participants.has(userId)) {
        throw new Error(`User not authorized for room: ${roomId}`);
   ...
   * Resume producer
   */
  async resumeProducer(roomId: string, producerId: string, userId: string) {
    try {
      const roomData = this.rooms.get(roomId);
      if (!roomData || !roomData.participants.has(userId)) {
        throw new Error(`User not authorized for room: ${roomId}`);
  ...
  getParticipants(roomId: string) {
    ...
    return Array.from(roomData.participants)
      .map((userId: any) => {
        const participantKey = `${roomId}_${userId}`;
        return this.participants.get(participantKey) || { userId, joinedAt: Date.now() };
      });
  }
   ...
   */
  async updateParticipantMediaState(roomId: string, userId: string, mediaState: any) {
    try {
      const participantKey = `${roomId}_${userId}`;
      const participantData = this.participants.get(participantKey);
   ...
   */
  getScreenSharing(roomId: string) {
   ...
   */
  isUserScreenSharing(roomId: string, userId: string) {
    const screenShareData = this.screenSharing.get(roomId);
