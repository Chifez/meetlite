...
import { memberService, type OrganizationMembersResponse, type InviteMemberRequest } from '../services/memberService';
...
export const useMembers = () => {
  ...
  // Invite a new member
  const inviteMember = useCallback(async (data: InviteMemberRequest) => {
    setInviting(true);
    try {
  ...
  // Remove a member
  const removeMember = useCallback(async (organizationId: string, memberId: string, memberName: string) => {
  ...
  const cancelInvitation = useCallback(async (organizationId: string, invitationId: string, email: string) => {
