...
import { Mic, MicOff, Video, VideoOff, PhoneOff, Users, Share, MoreVertical } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const Room = () => {
  ...
  useEffect(() => {
    ...
    const audioDeviceId = sessionStorage.getItem('meetlite_audio_device') || undefined;
    const videoDeviceId = sessionStorage.getItem('meetlite_video_device') || undefined;
  ...
  useEffect(() => {
    ...
    const handleCallUser = (data: { from: string; offer: RTCSessionDescriptionInit }) => {
    ...
    const handleAnswerMade = (data: { from: string; answer: RTCSessionDescriptionInit }) => {
  ...
  const shareScreen = async () => {
    try {
      ...
      if (localStream && videoTrack) {
        ...
        const promises = senders.map(sender => sender.replaceTrack(videoTrack));
        ...
        <div className="container max-w-4xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Button
              variant="outline"
              size="icon"
              className={`rounded-full h-12 w-12 ${!audioEnabled ? 'bg-destructive text-destructive-foreground hover:bg-destructive/90' : ''}`}
              onClick={toggleAudio}
            >
            ...
            <Button
              variant="outline"
              size="icon"
              className={`rounded-full h-12 w-12 ${!videoEnabled ? 'bg-destructive text-destructive-foreground hover:bg-destructive/90' : ''}`}
              onClick={toggleVideo}
            >
