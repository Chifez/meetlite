...
import { ArrowRight, Menu, X, Github, Star } from 'lucide-react';
      ...
      <header className="border-b border-border/60 bg-background/90 backdrop-blur-xl sticky top-0 z-30">
        <div className="relative z-20 flex items-center justify-between px-4 sm:px-6 lg:px-8 py-3 max-w-7xl mx-auto">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {NAV_LINKS.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="relative px-3.5 py-2 text-[0.8125rem] font-medium text-muted-foreground hover:text-foreground transition-all duration-200 tracking-[-0.01em] group"
                onClick={(e) => {
                  e.preventDefault();
              ...
              >
                {link.label}
                <span className="absolute bottom-1 left-3.5 right-3.5 h-[1.5px] bg-primary scale-x-0 group-hover:scale-x-100 transition-transform duration-200 origin-left" />
              </a>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {/* GitHub Stars Badge */}
            <a
              href="https://github.com/Chifez/meetlite"
              target="_blank"
              rel="noopener noreferrer"
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-foreground bg-muted/40 hover:bg-muted/80 border border-border/80 rounded-full transition-all duration-150 hover:border-primary/30 mr-1"
            >
              <Github className="w-3.5 h-3.5 text-muted-foreground" />
              <span>Star</span>
              <span className="flex items-center gap-0.5 text-muted-foreground border-l border-border/80 pl-1.5 ml-0.5 font-medium">
