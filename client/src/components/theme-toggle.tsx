...
const ThemeToggle = ({
  ...
  return (
    <Button
      ...
      onClick={handleToggle}
    >
      <Sun className="h-[1.2rem] w-[1.2rem] rotate-0 scale-100 transition-all duration-150 ease-out-strong dark:-rotate-90 dark:scale-95 dark:opacity-0" />
      <Moon className="absolute h-[1.2rem] w-[1.2rem] rotate-90 scale-95 opacity-0 transition-all duration-150 ease-out-strong dark:rotate-0 dark:scale-100 dark:opacity-100" />
      {children}
      <span className="sr-only">Toggle theme</span>
    </Button>
  );
